...
[{
	...
	if (GVAR(switchTank)) then {
		if !(isNil "GVAR(maxBar") then {
			...
			_class = format ["GVAR(tankSize%1", GVAR(selectedTank)];
			_maxBar = format ["GVAR(maxBar%1", GVAR(selectedTank)];

			_upperClass = "CfgWeapons";
			_obj = vest player;

			_tankSize = getNumber (configFile >> _upperClass >> _obj >> _class);
			GVAR(maxBar) = getNumber (configFile >> _upperClass >> _obj >> _maxBar);
			GVAR(filling) = _tankSize * GVAR(bar);
		};
