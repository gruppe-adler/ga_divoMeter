...
[{
	...
	(_displayUI displayCtrl 1132) ctrlSetText format ["%1", GVAR(selectedTank)];
	...
	if (((eyePos player select 2) < 0) && GVAR(ON)) then {
		...
		if (GVAR(METRIC)) then {
			...
			(_displayUI displayCtrl 1113) ctrlSetText format ["%1",(round(GVAR(depth) *10))/10];
			(_displayUI displayCtrl 1120) ctrlSetText "BAR";
			_bar = round(GVAR(filling)/100);
		}else {
			(_displayUI displayCtrl 1111) ctrlSetText "FT";
			(_displayUI displayCtrl 1113) ctrlSetText format ["%1",((round((GVAR(depth) * 3.28) *10))/10)];
		...
		if (GVAR(doDeco)) then {
			...
			if (!(GVAR(depth2deco) > 3) && !(GVAR(depth2deco) < -3)) then {
				(_displayUI displayCtrl 1120) ctrlSetText format ["%1", GVAR(decoTime)];
			};
		}else{
		...
		(_displayUI displayCtrl 1114) ctrlSetText format ["%1", round (((GVAR(timeleft)) + .01)/60)];
		(_displayUI displayCtrl 1119) ctrlSetText format ["%1", (round (GVAR(percentO2) * 100))/100];
		...
		switch (true) do {
			case (!GVAR(doDeco)): {
				...
				(_displayUI displayCtrl 1117) ctrlSetText format ["%1", round (GVAR(ascTime))];
				(_displayUI displayCtrl 1118) ctrlSetText format ["%1", round (((GVAR(diveTime))+.01)/60)];
			};
			case (GVAR(doDeco)) && !(GVAR(depth2deco)) < - 6) && !(GVAR(depth2deco)) > 6)): {
				...
				(_displayUI displayCtrl 1126) ctrlSetText format ["%1M", GVAR(depth2deco)];
				(_displayUI displayCtrl 1127) ctrlSetText format ["%1", GVAR(deepStopTime)];
				(_displayUI displayCtrl 1130) ctrlSetText QPATHTOF(images\triangle_down_divider.paa);
				(_displayUI displayCtrl 1131) ctrlSetText QPATHTOF(images\triangle_up_divider.paa);
			};
			case (GVAR(doDeepStop) && !(GVAR(depth2deepStop) < - 6) && !(GVAR(depth2deepStop) > 6)): {
				...
				(_displayUI displayCtrl 1126) ctrlSetText format ["%1M", GVAR(depth2deepStop]);
				(_displayUI displayCtrl 1127) ctrlSetText format ["%1", GVAR(deepStopTime)];

				switch (true) do {
					case (GVAR(depth2deepStop) > 1.5) : {
						if ((ctrlText 1131) == QPATHTOF(images\triangle_down_divider.paa)) then {
							(_displayUI displayCtrl 1130) ctrlSetText "";
							(_displayUI displayCtrl 1131) ctrlSetText "";
						}else{
							(_displayUI displayCtrl 1130) ctrlSetText "";
							(_displayUI displayCtrl 1131) ctrlSetText QPATHTOF(images\triangle_down_divider.paa);
						};
					};
					case (GVAR(depth2deepStop) < -1.5) : {
						if ((ctrlText 1131) == QPATHTOF(images\triangle_up_divider.paa)) then {
							(_displayUI displayCtrl 1130) ctrlSetText "";
							(_displayUI displayCtrl 1131) ctrlSetText "";
						}else{
							(_displayUI displayCtrl 1130) ctrlSetText QPATHTOF(images\triangle_up_divider.paa);
							(_displayUI displayCtrl 1131) ctrlSetText "";
						};
					};
		...
		if (GVAR(upDepth) <= 0.3) then {
			...
			if (GVAR(upDepth) >= 9) then {
				if ((ctrlText 1128) == QPATHTOF(images\left_06.paa))then {
					(_displayUI displayCtrl 1128) ctrlSetText "";
				}else{
					(_displayUI displayCtrl 1128) ctrlSetText QPATHTOF(images\left_06.paa);
				};
			}else{
				_getenhancedDivingRiseTexture = {
					...
					format[QPATHTOF(images\left_0%1.paa), _riseIndex];
				};
				(_displayUI displayCtrl 1128) ctrlSetText (GVAR(upDepth) call _getenhancedDivingRiseTexture);
			};
		};

		if (GVAR(narcFactor) >= 9.1) then {
				if ((ctrlText 1129) == QPATHTOF(images\right_09.paa))then {
					(_displayUI displayCtrl 1129) ctrlSetText "";
				}else{
					(_displayUI displayCtrl 1129) ctrlSetText QPATHTOF(images\right_09.paa);
				};
			}else {
				_getenhancedDivingN2Texture = {
					...
					format[ QPATHTOF(images\right_0%1.paa), _n2Index];
				};
			(_displayUI displayCtrl 1129) ctrlSetText (GVAR(narcFactor call _getenhancedDivingN2Texture);
		};
		(_displayUI displayCtrl 1132) ctrlSetText format ["%1", GVAR(selectedTank];
	}else{
		if (GVAR(METRIC)) then {
