...
if (_bool) then {
    diag_log format ["%1, %2, %3", (backpack ace_player) in GVAR(Tank), (backpack ace_player) in GVAR(Double), (vest ace_player) in GVAR(Rebreather)];
    if !((backpack ace_player) in GVAR(Tank) && {(backpack ace_player) in GVAR(Double)} && {(vest ace_player) in GVAR(Rebreather)}) exitWith {};
    private _value = _container getVariable QGVAR(DIVE_GAS1);

    if (isNil "_value")then {
       private _bar = getNumber (configFile >> "CfgVehicles" >> _obj >> QGVAR(maxBar1));
       private _class = getNumber (configFile >> "CfgVehicles" >> _obj >> QGVAR(gasClass1));
       private _o2 = -1;
       private _n2 = -1;
       private _he = -1;
    ...
    if ((_obj in GVAR(Double)) && GVAR(on)) then {
       ...
       if (isNil "_value")then {
          private _bar = getNumber (configFile >> "CfgVehicles" >> _obj >> "GVAR(maxBar2)");
