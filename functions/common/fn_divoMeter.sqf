...
[{
	...
	if ((underwater player) && DIVOMETERGEARON) then {	
		...
		grad_nTot = [grad_pressure, grad_percentN2, 1600, grad_tempC, 28, grad_pNAlv] call grad_divoMeter_fnc_gasCalc;
		grad_oTot = [grad_pressure, grad_percentO2, 756.7, grad_tempC, 32, grad_pOAlv] call grad_divoMeter_fnc_gasCalc;
		grad_HeTot = [grad_pressure, grad_percentHe, 2865, grad_tempC, 8, grad_pHeAlv] call grad_divoMeter_fnc_gasCalc;
		...
		// if (visibleWatch && DIVOMETERWATCHON) then {
				...
				case (grad_doDeepStop && !(grad_depth2deepStop < - 6) && !(grad_depth2deepStop > 6)): {
					...
					switch (true) do {
						case (grad_depth2deepStop > 1.5) : {
									if ((ctrlText 1131) == "grad_divoMeter\images\triangle_down_divider.paa") then {
										(_displayUI displayCtrl 1130) ctrlSetText "";
										(_displayUI displayCtrl 1131) ctrlSetText "";
									}else{
										(_displayUI displayCtrl 1130) ctrlSetText "";
						...
						case (grad_depth2deepStop < -1.5) : {
									if ((ctrlText 1131) == "grad_divoMeter\images\triangle_up_divider.paa") then {
										(_displayUI displayCtrl 1130) ctrlSetText "";
										(_displayUI displayCtrl 1131) ctrlSetText "";
									}else{
			...
			if (grad_upDepth <= 0.3) then {
				...
				if (grad_upDepth >= 9) then {
					if ((ctrlText 1128) == "grad_divoMeter\images\left_06.paa")then {
						(_displayUI displayCtrl 1128) ctrlSetText "";
					}else{
						(_displayUI displayCtrl 1128) ctrlSetText "grad_divoMeter\images\left_06.paa";
					};
				}else{
					_getDivoMeterRiseTexture = {
						...
						format[ "grad_divoMeter\images\left_0%1.paa", _riseIndex]; 
					}; 
					(_displayUI displayCtrl 1128) ctrlSetText (grad_upDepth call _getDivoMeterRiseTexture);
				};
			};
			
			if (grad_narcFactor >= 0.91) then {
				if ((ctrlText 1129) == "grad_divoMeter\images\right_09.paa")then {
					(_displayUI displayCtrl 1129) ctrlSetText "";
				}else{
					(_displayUI displayCtrl 1129) ctrlSetText "grad_divoMeter\images\right_09.paa";
				};
			}else {
				_getDivoMeterN2Texture = {
					...
					format[ "grad_divoMeter\images\right_0%1.paa", _n2Index]; 
				}; 
				(_displayUI displayCtrl 1129) ctrlSetText (grad_narcFactor call _getDivoMeterN2Texture);
			};
		...
		if (grad_dDepth > 9.1 && grad_maxDepth >= 20) then {						
			...
			if (grad_fAscCntdn == 0) then {
				[(1-(grad_nTisTot *1.25))]call grad_divoMeter_fnc_DCSEffects;					
				grad_fAscCntdn = 5;
			};					
		}else {
			grad_fAscCntdn = 1;
		...
		grad_nTisTot = [grad_percentN2, ((ln 2)/4), grad_pressure, ((grad_pressureB - grad_pressureA)/60), grad_diveTime] call grad_divoMeter_fnc_initTissues;
		grad_HeTisTot = [grad_percentHe, ((ln 2)/4), grad_pressure, ((grad_pressureB - grad_pressureA)/60), grad_diveTime] call grad_divoMeter_fnc_initTissues;
		grad_O2TisTot = [grad_percentO2, ((ln 2)/4), grad_pressure, ((grad_pressureB - grad_pressureA)/60), grad_diveTime] call grad_divoMeter_fnc_initTissues;
		...
		if (grad_narcFactor > 8) then {
			...
			if (grad_narcCntdn == 0) then {
				[(grad_narcFactor/50)] call grad_divoMeter_fnc_narcEffects;					
			grad_narcCntdn = 33;
			};					
		}else {
			grad_narcCntdn = 1;
		...
		if (grad_pHeAlv > 8) then {	
			...
			if (grad_HeToxCntdn == 0) then {
				[] call grad_divoMeter_fnc_HeToxEffects;					
				grad_HeToxCntdn = 33;
			};				
		}else {
			grad_HeToxCntdn = 1;
		...
		if (grad_depth > ((grad_maxppO/grad_percentO2) -1) *10) then {	
			...
			if (grad_O2ToxCntdn == 0) then {
				[grad_O2DamMult] call grad_divoMeter_fnc_O2ToxEffects;					
				grad_O2ToxCntdn = 33;
			};
		}else {
			grad_O2ToxCntdn = 1;
