params ["_HeToxRunScript", "_HeToxRunEffect"];
_HeToxRunTimes = 0;

_handle = [{
	params ["_args","_handle"];  
	_args params ["_HeToxRunEffectt", "_HeToxRunTimes"];
	
	if (!_HeToxRunScript) exitWith {[_handle] call CBA_fnc_removePerFrameHandler};;
